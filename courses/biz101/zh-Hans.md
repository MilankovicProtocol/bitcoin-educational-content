...
当顾客在商店使用信用卡时，POS终端会读取信用卡，并将交易数据安全地传输给商家的收单银行。收单银行将此信息转发给相关的银行卡网络（如Visa卡或Mastercard卡），后者再将请求转发给发卡行，即为顾客提供信用卡的银行。发卡行将检查客户的账户或信用额度，并通过网络和收单银行发回授权，允许商户接受付款。
