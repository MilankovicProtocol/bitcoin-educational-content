...
| 支付方式                     | 是否需要授权？                 | 交易批准时间（商户视角）              | 结算速度（资金完全结算）                  | 最终性（撤销难易程度）                  | 中介数量                       | 典型费用（收款人）               |
| --------------------------- | ---------------------------- | ---------------------------------- | ------------------------------------- | ---------------------------------- | ---------------------------- | ---------------------------- |
| **现金（Cash）**            | 否                          | 即时（物理交换）                   | 即时（无结算延迟）                     | 高（支付后不可撤销）                  | 无                           | 无                           |
| **支票（Checks）**           | 是（银行清算）               | 存款时接受（不保证）               | 数天（支票清算过程）                   | 中（在清算前可能被拒绝/停止）         | 银行                         | **低到中等**（银行费用）       |
| **电汇（Wire Transfers）**     | 是（银行/网络）              | 几小时内确认                      | 当天或次日（国内）                     | 高（通常一旦发送不可逆）               | 银行、支付网络               | **中等**（固定/百分比）         |
| **支付卡（Payment Cards）**    | 是（发卡机构授权）           | 几秒到几分钟（授权代码）           | 数天（银行间结算）                     | 中（可能出现拒付）                   | 发卡行、收单行、卡网络       | **可变（交易金额的1-3%）**     |
| **数字钱包/移动支付（Digital Wallets/Mobile Pay）** | 是（钱包提供商/银行）             | 几秒钟（即时确认）                   | 通常1-2天（取决于资金来源）            | 中（可能存在退款/争议）             | 银行、钱包运营商             | **低到中等（因情况而异）**     |
...
| 限制                         | 解释                                                                                                                                                                            | 影响                                                                                           |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| 高昂的卡片费用              | 交换费（约0.3%）、网络费用（固定或0.3%-1%）、终端/PSP订阅费以及银行利润（0.5%-1.7%）加起来构成了巨大的成本——类似于对生产性部门征收的全球性“税收”，总计数万亿美元。 | 增加商户成本，减少利润率，并可能推高消费者价格。                                               |
| 非常缓慢的最终结算           | 资金结算可能需要长达5天的时间，减缓了资金流动和整体经济活动的速度。                                                                                                            | 延迟商户的流动性并降低了经济流通的速度。                                                      |
| 欺诈风险                   | 电子商务渠道频繁成为欺诈的目标，导致巨额损失（例如，280亿美元）。到2024年，全球拒付金额可能达到约1740亿美元。处理这些争议耗费时间并造成精神压力。               | 增加运营成本、复杂的防欺诈措施，并降低了客户的信任。                                           |
| 购物车放弃                  | 额外的安全步骤（一次性代码、PSD2下的双重身份验证）在结账过程中引入了摩擦。                                                                                                  | 更高的结账复杂性导致购物车放弃率增加，销售损失。                                               |
| 高最低交易金额               | 卡片的最低消费门槛可能迫使商户和消费者在不方便的定价或购买条件下进行交易，从而阻碍了小额交易。                                                                                   | 减少了客户的满意度和灵活性，可能限制冲动购买或低价值交易。                                      |
| 缓慢的预授权                | 当前系统无法处理以毫秒级速度进行的交易或支持连续的、实时的支付流。                                                                                                            | 限制了需要即时或流式支付的用例，限制了创新和可扩展性。                                          |
| 需要银行/卡账户             | 使用这些支付方式需要绑定的银行或卡账户，这自动排除了那些没有这些账户的人。                                                                                                       | 限制了金融包容性，减少了未银行化或银行服务不足人群的使用可能性。                                |
| 重复的在线账户创建           | 用户通常必须创建多个在线账户，这导致疲劳、便利性下降并增加了个人数据的暴露。                                                                                                 | 恶化用户体验，增加隐私问题，并提高数据泄露的风险。                                               |
...
| 属性                           | 入门者                                                                                                          |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| **心态**                       | “尝试我的第一个实物支付”，“为我的在线内容接受小费”，“目标是非常小的收入”                                           |
| **交易频率**                   | “为了学习而进行的第一次交易”，“偶尔接受支付”                                                                      |
...
| **费用**                       | 客户支付比特币闪电网络费用及任何适用的应用费用                                                                   |
| **销售点设备**                 | 免费智能手机应用程序或可选的物理终端（例如 Bitcoinize）                                                         |
| **管理与角色**                 | 单个应用程序管理；角色区分最小化                                                                                 |
| **会计导出**                   | 基本的交易历史列表                                                                                             |
| **API**                        | 否                                                                                                              |
...
| 属性                           | 基本用户                                                                                                        |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------- |
| **心态**                       | “我接受比特币作为我的业务支付方式，但我不期望有大量交易量”                                                       |
| **交易频率**                   | 每月几笔交易                                                                                                    |
...
| **商户界面**                   | 简单的比特币闪电网络钱包：手机上的应用程序；简单的发票开具，详细信息最少                                         |
| **客户界面**                   | 比特币二维码支付码，通过客户的个人钱包扫描                                                                      |
...
| 属性                           | 专业用户                                                                                                           |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| **心态**                       | - 像任何其他支付方式一样的电子商务支付方式 - 或者为一组准备好更大交易量的企业提供联合管理                           |
| **交易频率**                   | 每天多次交易                                                                                                       |
...
| **客户界面**                   | 比特币二维码支付码，通过客户的个人钱包扫描                                                                         |
| **费用**                       | 免费开源后端与付费闪电网络托管/服务费用的混合；前端费用包括比特币闪电网络费用和小于1.5%的转换费                     |
...
| **支付类型**                   | 任何规模或数量                                                                                                    |
| **设置复杂性**                 | 中到高，取决于架构选择                                                                                           |
| **推荐解决方案示例**           | 定制架构或SaaS托管解决方案的编排，可能使用第三方LSP（闪电网络服务提供商）服务                                      |
| **商户界面**                   | 完全定制化的前端和后端接口，完全集成到企业的工作流和流程中                                                        |
| **客户界面**                   | 从比特币二维码支付码到完全定制化的用户界面和/或API集成                                                           |
| **费用**                       | 内部开发与第三方费用的组合；客户支付比特币闪电网络费用以及服务提供商的任何交易费用                                 |
| **销售点设备**                 | 定制化的解决方案，适用于企业环境                                                                                  |
